---
name: topic-creator
description: Scaffold a new research topic in topics/topics.json and topics/<id>/ with subject_matter.yaml, exclusions.yaml, assets, and MongoDB db wiring. Use when the user asks to add a topic, switch subject matter, or create topic configuration.
---

# Topic creator

Add a new entry to the multi-topic research pipeline. Each topic owns its YAML prompts, exclusions, UI assets, and a **MongoDB database** named by the `db` property.
...
6. **Environment** — optional overrides in `.env`:
   ...
   - Per-file overrides still work: `SUBJECT_MATTER_CONFIG`, `EVENT_EXCLUSIONS_CONFIG`
   - `SCHEDULE_INTERVAL_HOURS` — hours between `agent serve` runs (default 1)
