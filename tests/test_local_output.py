"""Tests for the accumulating spreadsheet output (Tasks 8 + 9)."""
...
from agent.local_output import RESEARCH_FILENAME, _IDX_URL, load_spreadsheet_resources, merge_and_write
...
def test_duplicate_url_skipped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("agent.config.OUTPUT_DIR", tmp_path)
...
def test_load_spreadsheet_resources_roundtrip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Resources written to the spreadsheet should be readable back out."""
    monkeypatch.setattr("agent.config.OUTPUT_DIR", tmp_path)
    ...
    assert "https://example.com/mic" in urls
    # Titles should be reconstructed in "Act @ Venue, Location" format
