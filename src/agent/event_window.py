...
def curator_date_instruction(*, days: int = DEFAULT_EVENT_HORIZON_DAYS) -> str:
    ...
    return (
        ...
        "Each row must be one specific event with a direct link to that event — "
        "not venue homepages, not festival index pages, not \"what's on\" portals "
        "unless the URL is clearly for one named show with a date. "
        "Set field date to ISO YYYY-MM-DD (the main show day) so results can be sorted; "
        "omit rows without a confident date in that window.\n"
